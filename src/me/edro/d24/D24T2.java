...
public class D24T2 {
    ...
    public static void main(String[] args) {
        ...
        LinkedList<Coords> targets = new LinkedList<>(Arrays.asList(
                new Coords(mapWidth - 2, mapHeight - 1),
                new Coords(1, 0),
                new Coords(mapWidth - 2, mapHeight - 1)
        ));
        ...
        while (true) {
            ...
            for (Coords c : moves) {
                // finish condition
                if (c.x == targets.getFirst().x && c.y == targets.getFirst().y) {
                    finish = true;
                }
                ...
                if (
                        currentBlizzards.contains(c)
                                ...
                                || c.x >= mapWidth - 1
                                || c.y <= 0 && c.x != 1
                                || c.y >= mapHeight - 1 && c.x != mapWidth - 2
                ) {
            ...
            if (finish) {
                System.out.println("Reached target " + targets.peekFirst());
                targets.remove();
                if (targets.size() == 0){
                    break;
                }
